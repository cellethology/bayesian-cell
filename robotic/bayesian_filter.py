...
class BayesianFilter:
    ...
    def update_belief(self, measurement, robot_pos):
        ...
        estimated_target_pos = np.unravel_index(np.argmax(self.belief), self.belief.shape)
        ...
        df_dx, df_dy = self.signal_model.get_signal_gradient(robot_pos, estimated_target_pos)
    ...
    def motion_update(self, current_signal, current_pos, adaptive_process_variance):
        ...
        if adaptive_process_variance == "exponential":
            ...
            action = self.motion_model.get_next_intended_action(current_pos, self.belief)
        ...
        kernel = np.exp(-(self.motion_model.kernel_mat**2) / (2 * adaptive_sigma**2))
        kernel /= kernel.sum()
        
        # Use FFT-based convolution for better performance with larger grids
        if self.config["grid_size"] > 50:
