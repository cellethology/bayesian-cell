"""
FilterPy-based Extended Kalman Filter implementation for target tracking.
Uses the robust and well-tested FilterPy library instead of custom implementation.
"""
...
class FilterPyExtendedKalmanFilter(BaseFilter):
    """Extended Kalman Filter using FilterPy library for target tracking."""
    ...
    def __init__(self, config):
        ...
        # Measurement: scalar signal strength
        self.filterpy_ekf = FilterPyEKF(dim_x=2, dim_z=1)
        ...
        # Process model (stationary target, so F = I)
    ...
    def _hx(self, x):
        """
        Measurement function for FilterPy.
    ...
    def _HJacobian(self, x):
        """
        Jacobian of measurement function for FilterPy.
    ...
    def get_innovation_stats(self):
        ...
        if hasattr(self.filterpy_ekf, 'y') and hasattr(self.filterpy_ekf, 'S'):
            return {
                ...
                'mahalanobis_distance': self.filterpy_ekf.log_likelihood if hasattr(self.filterpy_ekf, 'log_likelihood') else None
            }
        return None