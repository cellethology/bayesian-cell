...
class BayesianNavigation:
    def __init__(self, config=None):
        ...
        self.innovation_history = (
            []
        )  # Store innovation (measurement - prediction) history
        self.predicted_variance_history = (
            []
    ...
    def get_optimal_motion_kernel(self, theta_0, adaptive_sigma):
        ...
        The kernel is centered on the intended displacement and reflects the filter's current uncertainty.
        """
        ...
        pos = np.stack([x, y], axis=-1)  # shape (size, size, 2)
    ...
    def compute_all_expected_signal(self, target_pos):
        ...
        signal = self.get_expected_signal(xx, yy, target_pos)
        return signal
    ...
    def _diag_state_cov(self):
        """Var(x), Var(y) of the posterior belief – needed for H P Hᵀ term."""
        idx = np.indices(self.belief.shape)
        μx = np.sum(idx[0] * self.belief)
        μy = np.sum(idx[1] * self.belief)

        var_x = np.sum(((idx[0] - μx) ** 2) * self.belief)
        var_y = np.sum(((idx[1] - μy) ** 2) * self.belief)
        return var_x, var_y
    ...
    def update_belief(self, measurement, robot_pos):
        ...
        # Store innovation and predicted variance for adaptation
        ...
        if len(self.innovation_history) > window_size:
            self.innovation_history = self.innovation_history[-window_size:]
            self.predicted_variance_history = self.predicted_variance_history[
                -window_size:
            ]
    ...
    def get_likelihood(self, measurement, expected_signal_grid):
        if self.config["noise_model"] == "poisson":
            ...
            likelihood = np.exp(
                -((measurement - expected_signal_grid) ** 2)
                / (2 * self.measurement_variance)
    ...
    def update_process_variance_from_position_error(self, intended_pos, actual_pos):
        """
        Update process variance based on observed motion error.
        Uses squared Euclidean distance between intended and actual position as a proxy for variance.
        """
        dx = actual_pos[0] - intended_pos[0]
        dy = actual_pos[1] - intended_pos[1]
